    ...
    >
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        <div className="space-y-2">
          <h2 className="font-heading text-2xl md:text-3xl leading-tight font-semibold tracking-tight text-[#79542e] flex items-center gap-3">
            {project.nombre}
            <span className="w-12 h-1 bg-[#79542e] rounded-full hidden md:block" />
          </h2>
          {project.subtitulo && <p className="text-secondary font-medium">{project.subtitulo}</p>}
        </div>

        <Button
          render={project.tourUrl ? <a href={project.tourUrl} target="_blank" rel="noreferrer" /> : undefined}
          className="flex items-center gap-2 px-6 py-3 h-auto rounded-xl font-bold hover:scale-105 transition-all shadow-md active:scale-95 group bg-[#79542e] text-secondary-foreground hover:bg-[#A67C52]"
        >
      ...
      {project.lotes.length === 0 ? (
        <p className="text-sm text-muted-foreground italic">Sin lotes cargados todavía.</p>
      ) : (
        <>
            ...
            <table className="w-full min-w-[800px] border-collapse">
              <thead>
                <tr className="bg-secondary text-white text-sm font-bold uppercase tracking-wider divide-x divide-white/20">
                  ...
                  <th className="py-5 px-6 text-left">Superficie M²</th>
                  <th className="py-5 px-6 text-left">Precio Contado</th>
                  <th className="py-5 px-6 text-left">Pie 50% + 11 Cuotas</th>
                  <th className="py-5 px-6 text-left">Pie 50% + 24 Cuotas</th>
                  <th className="py-5 px-6 text-left bg-tertiary">Precio Lista</th>
                </tr>
              </thead>
              <tbody className="text-sm text-foreground">
                {project.lotes.map((lote) => (
                  ...
                  >
                    ...
                    <td className="py-5 px-6 text-muted-foreground">{lote.precioContado}</td>
